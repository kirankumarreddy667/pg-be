...
> {
    ...
    const animals = await this.getUserAnimals(user_id);
    if (animals.length === 0) {
        return []
    }

    const allData = await this.getAnimalPregnancyData(user_id);
    const animalGroups = this.groupAnimalsById(allData);
    this.processAnimalGroups(animalGroups, resData);

    return Object.keys(resData).length === 0 ? [] : resData
}

private static async getUserAnimals(user_id: number): Promise<{ animal_number: string; animal_id: number }[]> {
    return (await db.sequelize.query(
        `
        SELECT DISTINCT animal_number, animal_id
        FROM animal_question_answers 
    ...
    )) as unknown as { animal_number: string; animal_id: number }[];
}

private static async getAnimalPregnancyData(user_id: number): Promise<{
    animal_id: number
    animal_number: string
    animal_name: string
...
private static groupAnimalsById(allData: {
    animal_id: number
    animal_number: string
    animal_name: string
    sex_answer: string | null
    pregnancy_answer: string | null
}[]): Map<number, typeof allData> {
...
private static processAnimalGroups(
    animalGroups: Map<number, {
        animal_id: number
        animal_number: string
        animal_name: string
        sex_answer: string | null
    ...
    resData: Record<string, { pregnant_animal: number; non_pregnant_animal: number }>
): void {
    for (const animalNumbers of animalGroups.values()) {
        ...
        for (const row of animalNumbers) {
            if (this.isValidFemaleAnimal(row)) {
                if (row.pregnancy_answer) {
                    if (row.pregnancy_answer.toLowerCase() === 'yes') {
                        pregnantAnimal += 1
                    } else {
                        nonPregnantAnimal += 1
                    }
...
private static isValidFemaleAnimal(row: {
    sex_answer: string | null
    pregnancy_answer: string | null
}): boolean {
    return row.sex_answer !== null && row.sex_answer.toLowerCase() !== 'male';
}
